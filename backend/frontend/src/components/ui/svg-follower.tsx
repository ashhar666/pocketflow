...
export function SVGFollower({
  ...
  useEffect(() => {
    // Initialize followers immediately if svg is ready
    if (svgRef.current && followersRef.current.length === 0) {
      followersRef.current = colors.map((color) => new Follower(svgRef.current!, color))
    }

    const onMove = (e: MouseEvent) => handleMouseMove(e)
    const onTouch = (e: TouchEvent) => handleTouchMove(e)
  ...
  }, [colors, handleMouseMove, handleTouchMove])
