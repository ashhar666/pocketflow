          ...
          <div className="md:hidden sticky top-0 z-[100] flex items-center justify-between mb-8 bg-background/80 backdrop-blur-xl p-4 rounded-b-2xl border-b border-x border-black/5 dark:border-white/10 shadow-sm print:hidden">
            <div className="flex items-center gap-3">
              ...
              <div className="overflow-hidden">
                <p className="text-xs font-bold text-foreground uppercase truncate tracking-tight">{user?.first_name || user?.username}</p>
            ...
            <div className="flex items-center gap-3">
              ...
              <motion.button
                ...
                className="px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-zinc-600 hover:text-emerald-500 hover:bg-emerald-500/10 rounded-xl transition-all duration-300 border border-black/5 dark:border-white/5 shadow-sm"
              >
                Menu
              </motion.button>
            </div>
          ...
          <AnimatePresence>
            {isMenuOpen && (
              <>
                ...
                >
                  <div className="p-6 border-b border-black/5 dark:border-white/10 flex items-center justify-center bg-zinc-500/5 dark:bg-black">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
                    <div className="px-2 mb-2">
                      <span className="text-[9px] font-bold uppercase tracking-[0.35em] text-zinc-500">More Links</span>
                    </div>
                    {navItems.slice(5).map((item) => {
                      ...
                      return (
                        <Link 
                          ...
                          href={item.href}
                          onClick={() => setIsMenuOpen(false)}
                          className={`flex items-center gap-4 p-4 rounded-2xl transition-all ${isActive ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-black/[0.02] dark:bg-black text-zinc-500 hover:text-foreground'}`}
                        >
                  ...
                  <div className="p-4 border-t border-black/5 dark:border-white/10 bg-zinc-500/5 dark:bg-black">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-3 px-3 py-3 text-[10px] font-bold uppercase tracking-widest text-red-500 hover:bg-red-500/10 rounded-xl transition-all duration-300 border border-red-500/20 shadow-sm"
                    >
                      Logout
                    </button>
                  </div>
